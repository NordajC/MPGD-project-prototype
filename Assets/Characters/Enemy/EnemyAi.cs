...
public class EnemyAi : MonoBehaviour, ICombat
{
    ...
    public virtual void onHitReaction(bool heavyAttack, Vector3 hitLocation, HitDirection hitDirection, HitHeight hitHeight, GameObject receivedFrom, float damage)
    {
        // Play hit sound
        AudioSource audioSource = receivedFrom.GetComponent<PlayerCombat>().audioSource;
        WeaponryItem weaponryItem = receivedFrom.GetComponent<PlayerInventory>().playerWeaponPrimary.itemTemplate as WeaponryItem;
        AudioClip[] hitSounds = weaponryItem != null ? weaponryItem.hitSounds : receivedFrom.GetComponent<PlayerCombat>().hitSounds;
        Manager.playRandomSound(ref audioSource, hitSounds, 0.3f);
