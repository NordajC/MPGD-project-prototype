...
public class PlayerInventory : MonoBehaviour
{
    ...
    public GameObject pickaxeHitPartice;
    ...
    public void equipItem(int movedFromSlot, string droppedSlotName)
    {
        ...
        Manager.playSound(ref interactSection.audioSource, interactSection.inventoryEquip, 0.2f);
    ...
    public void swapPrimarySecondaryWeapon()
    {
        ...
        GetComponent<MovementStateManager>().enabled)
        {
            if(playerWeaponPrimary.itemTemplate.ItemId != 0 || playerWeaponSecondary.itemTemplate.ItemId != 0)
            {
                Manager.playSound(ref interactSection.audioSource, interactSection.inventoryEquip, 0.2f);
            }
    ...
    public void OnToggleCrafting(GameObject craftScreen)
    {
    ...
    {
        Instantiate(pickaxeObject, GameObject.Find("Male_Weapon_Tool_Left").transform);
    }

    public void toggleAxe()
    {
        Instantiate(axeObject, GameObject.Find("Male_Weapon_Tool_Left").transform);
    }

    public void onToolHit()
    {
        Rock rock = GetComponent<Interaction>().interactedItem as Rock;
        rock.TakeDamage(10f);
        rock.togglePlayerMovement(true);
    }

    public void onToolHitEnd()
    {
        if(GameObject.Find("Male_Weapon_Tool_Left").transform.childCount > 0)
        {
            Destroy(GameObject.Find("Male_Weapon_Tool_Left").transform.GetChild(0).gameObject);
        }
    }

    public void pickaxeParticle()
    {
        Instantiate(pickaxeHitPartice, GameObject.Find("ToolHitLocation").transform.position, Quaternion.identity);
    }
}
